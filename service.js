		...
		request('https://api.bf4stats.com/api/playerInfo?plat=pc&name='+req.name+'&output=js', function (error, response, body) {
			if(error){
					console.log('error');
					  ...
					  result.push(splitByComma[1]); 
					  result.push(splitByComma[2]);
					  result.push(splitByComma[3]); 
					  result.push(splitByComma[4]);
					  result.push(splitByComma[12]); 
					  result.push(splitByComma[13]);		
